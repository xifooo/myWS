...
# 在学习过程中不懂的点就回去看廖老师的教程, 然后就发现, 之前抽象的现在就能懂一些了
...
'''
loop自Python3.5以后就被废除了, 故项目中所有的loop都得删掉
'''
...
async def select(sql, args, size=None):
    ...
    with (await __pool) as conn:
        ...
        cur = await conn.cursor(aiomysql.DictCursor) # 将调用一个子协程()也就是在一个协程中调用另一个协程)并直接获得子协程的返回结果
...
'''
注意到Model只是一个基类, 
要将具体的子类如User的映射信息读取出来需要通过metaclass: ModelMetaclass。 
这样, 任何继承自Model的类(比如User), 
会自动通过ModelMetaclass扫描映射关系, 
并存储到自身的类属性如__table__、__mappings__中。缀加以下代码至 Model 代码之前: 
'''
...
# 参考廖老师教程:  https://www.liaoxuefeng.com/wiki/1016959663602400/1017592449371072
